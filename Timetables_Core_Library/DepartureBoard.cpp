...
std::vector<Timetables::Structures::Departure> Timetables::Algorithms::GetDepartureBoard(const Timetables::Structures::GtfsFeed & feed, const std::wstring & stationName, const Timetables::Structures::Datetime & datetime, const size_t count) {
	...
	for (auto&& child_stop : station.GetChildStops()) {
		const multimap<Time, StopTimePtrObserver>& stopDep = child_stop->GetDepartures(); // TO-DO: Myslím, že toto je špatnì. Problém s mergováním èasù typu 25:xx a 0:xx.

		auto firstRelevant = stopDep.upper_bound(departureTime); // TO-DO: Co když narazíme na cend...
	...
	auto firstRelevant = departures.upper_bound(departureTime); // TO-DO: Co když narazíme na cend...
